pipeline {
    agent any
    environment {
        DOCKER_IMAGE = "Yugeshwarangm/docker-app:latest"  // Change this to your registry
        ...
        stage('Checkout Code') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'github-yugeshwaran', usernameVariable: 'GIT_USER', passwordVariable: 'GIT_TOKEN')]) {
                    git url: "https://$GIT_USER:$GIT_TOKEN@github.com/Yugeshwaran-gm/devops-training.git", branch: 'main'
                }
            }
        }
